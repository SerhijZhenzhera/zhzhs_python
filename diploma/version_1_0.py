...
def soccer_team(name, current_round):
    ...
    soccer_mood(a_match_team, b_match_team, name, current_round)


def soccer_mood(a_match_team, b_match_team, name, current_round):
...
def soccer_gra(pl_match_team, cm_match_team, name, current_round):
    result_player = 0
    result_computer = 0
    ...
    for minute in range(1, 92):
        if minute >= 5:
            ...
            if result_player > result_computer:
                ...
                soccer_end()
                # return вызывает продолжение матча (из-за: result_3 = False ?)
