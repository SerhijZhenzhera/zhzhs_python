...
class Address():
    ...
    @property
    def validate(self):
        if not type(self.e_mail) == str:
            print('Not a string!')
            return False
        elif not self.e_mail.count('@') == 1:
            print('there should be one and only one "@"')
            return False
        else:
            at = self.e_mail.find('@')
            local_part = self.e_mail[:at]
            domain_part = self.e_mail[at+1:]
            quoted = ''
            ...
            if '(' and ')' and ',' and ':' and ';' and '<' and '>' and '[' and '\\' and ']' and '..' in self.e_mail:
